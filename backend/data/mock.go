...
func (db *DbMock) ReadsGet(username string) ([]ReadWithId, error) {
	user, err := db.selectUsers(username)
	if err != nil {
		return nil, err
	}
...
func (db *DbMock) ReadGet(readId string) (Read, error) {
	var read DbRead
	for _, candidate := range db.reads {
		if candidate.ReadId == readId {
			read = candidate
			break
		}
	}
